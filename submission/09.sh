...
UTXO_TXID=$(echo "$TXID" | tr -d '=')
TX_INPUTS="[ {\"txid\": \"$UTXO_TXID\", \"vout\": $UTXO_VOUT_INDEX, \"sequence\": 4294967294} ]"
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address == "'$PAYMENT_ADDRESS'" or .scriptPubKey.addresses[0] == "'$PAYMENT_ADDRESS'") | .value')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address == "'$CHANGE_ADDRESS'" or .scriptPubKey.addresses[0] == "'$CHANGE_ADDRESS'") | .value')
...
SIMPLE_TX_INPUTS='[{"txid":"'$CLEAN_TXID'","vout":0,"sequence":4294967293}]'
...
# WRITE YOUR SOLUTION BELOW:
if [[ $(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq -r '.vout[0].scriptPubKey.addresses[0]') == "$CHANGE_ADDRESS" ]]; then
...
CHANGE_AMOUNT_SATS=$(echo "$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq -r '.vout['$CHANGE_OUTPUT_INDEX'].value') * 100000000" | bc | sed 's/\..*//g')
CHILD_SEND_AMOUNT=$((CHANGE_AMOUNT_SATS - CHILD_FEE_SATS))
