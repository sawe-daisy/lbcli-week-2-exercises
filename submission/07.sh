#!/bin/bash

BITCOIN_CLI="bitcoin-cli -regtest"
RECIPIENT="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
AMOUNT_SATS=20000000 
FEE_SATS=1000        
SATOSHIS_PER_BTC=100000000

AMOUNT_BTC=$(awk "BEGIN { printf \"%.8f\", $AMOUNT_SATS / $SATOSHIS_PER_BTC }")
FEE_BTC=$(awk "BEGIN { printf \"%.8f\", $FEE_SATS / $SATOSHIS_PER_BTC }")

RAW_TX=""

UTXO_TXID=$($BITCOIN_CLI decoderawtransaction "$RAW_TX" | jq -r '.vin[0].txid')
UTXO_VOUT=$($BITCOIN_CLI decoderawtransaction "$RAW_TX" | jq -r '.vin[0].vout')

TOTAL_INPUT_BTC=$($BITCOIN_CLI decoderawtransaction "$RAW_TX" | jq '[.vout[].value] | add')

CHANGE_BTC=$(awk "BEGIN { printf \"%.8f\", $TOTAL_INPUT_BTC - $AMOUNT_BTC - $FEE_BTC }")

CHANGE_ADDRESS=$($BITCOIN_CLI getrawchangeaddress)

RAW_TX_HEX=$($BITCOIN_CLI createrawtransaction \
  "[{\"txid\":\"$UTXO_TXID\",\"vout\":$UTXO_VOUT}]" \
  "{\"$RECIPIENT\":$AMOUNT_BTC, \"$CHANGE_ADDRESS\":$CHANGE_BTC}")

echo "$RAW_TX_HEX"


# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# raw_tx=""
